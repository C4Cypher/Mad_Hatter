...
%-----------------------------------------------------------------------------%
% 
% File: mh_ordered_proposition_set.m
% Main author: C4Cypher.
% Stability: low.
%-----------------------------------------------------------------------------%

:- module mh_ordered_proposition_set.
...
:- type mh_ordered_proposition_set. 
% where comparison is compare_ordered_proposition_sets.
...
% Basic operations
	...
	% Array
:- pred is_valid(mh_ordered_proposition_set::in) is semidet.

:- func empty_set = mh_ordered_proposition_set.

:- pred empty_set(mh_ordered_proposition_set::out) is det.

:- pred is_empty(ordered_proposition_set::in) is semidet.
...
:- pred is_sorted(mh_ordered_proposition_set::in) is semidet.

	% Sort a given set using the standard ordering without removing elements.
:- func sort(mh_ordered_proposition_set) = mh_ordered_proposition_set.
:- pred sort(mh_ordered_proposition_set::in, 
	mh_ordered_proposition_set::out) is det.

	% Sort using the standard ordering and remove duplicaates
:- func sort_and_remove_dups(mh_ordered_proposition_set) 
	= mh_ordered_proposition_set.
:- pred sort_and_remove_dups(mh_ordered_proposition_set::in,
	mh_ordered_proposition_set::out) is det.
	
:- func remove_dups(mh_ordered_proposition_set) = mh_ordered_proposition_set.
:- pred remove_dups(mh_ordered_proposition_set::in, 
	mh_ordered_proposition_set::out) is det.

	% Return the number of elements in the ordered portion of the set
:- func length(mh_ordered_proposition_set) = int.
:- pred length(mh_ordered_proposition_set::in, int::out) is det.


	% Return the number of unique elements in the set
:- func size(mh_ordered_proposition_set) = int.
:- pred size(mh_ordered_proposition_set::in, int::out) is det.
...
:- pred equal(mh_ordered_proposition_set::in, 
	mh_ordered_proposition_set::in) is semidet.

	% Succeeds if the sets have the same values, with duplicates removed
	% Equivalent to compare_ordered_proposition_sets((=)).
:- pred equivalent(mh_ordered_proposition_set::in, 
	mh_ordered_proposition_set::in) is semidet.
...
:- pred compare_ordered_proposition_sets(comparison_result::uo, 
	mh_ordered_proposition_set::in, mh_ordered_proposition_set::in) is det.

%-----------------------------------------------------------------------------%
% Conversion

:- func from_ordered_set(ordered_set(mh_proposition))
	= mh_ordered_proposition_set.
:- func to_ordered_set(mh_ordered_proposition_set)
	= ordered_set(mh_proposition).

:- func from_list(list(mh_proposition)) = mh_ordered_proposition_set.
:- func to_list(mh_ordered_proposition_set) = list(mh_proposition).

:- func from_array(array(mh_proposition)) = mh_ordered_proposition_set.
:- func to_array(mh_ordered_proposition_set) = array(mh_proposition).

	% Sorts and removes duplicates (if any) from the set, the assigned ordering
	% is ignored
:- func to_sorted_list(mh_ordered_proposition_set) = list(mh_proposition).
:- func to_sorted_array(mh_ordered_proposition_set) = array(mh_proposition).

:- func to_proposition_set(mh_ordered_proposition_set) = mh_proposition_set.
...
:- pred bounds(mh_ordered_proposition_set::in, int::out, int::out) is det.

	% As above, but fail on emtpy set
:- pred semidet_bounds(mh_ordered_proposition_set::in, int::out, int::out)
	is semidet.

	% Succeed if the given index is in bounds for the given ordered_proposition_set
:- pred in_bounds(mh_ordered_proposition_set::in, int::in) is semidet.

	% Return the first index (should always be 1) unless the set is empty 
	% If empty return -1
:- func min(mh_ordered_proposition_set) = int is det.
:- pred min(mh_ordered_proposition_set::in, int::out) is det.

	% Fails if empty
:- func semidet_min(mh_ordered_proposition_set) = int is semidet.
:- pred semidet_min(mh_ordered_proposition_set::in, int::out) is semidet.

	% Return the last index, -1 if empty
:- func max(ordered_proposition_set) = int is det.
:- pred max(ordered_proposition_set::in, int::out) is det.

:- func semidet_max(ordered_proposition_set) = int is semidet.
:- pred semidet_max(ordered_proposition_set::in, int::out) is semidet.

:- pred contains(mh_ordered_proposition_set::in, mh_proposition::in)
	is semidet.

	% Lookup the ordered value at the given index (starting at 1), throws an	% exception if index is out of bounds
:- func lookup(mh_ordered_proposition_set, int) = mh_proposition is det.
:- pred lookup(mh_ordered_proposition_set::in, int::in, mh_proposition::out)
	is det.

	% Skips bounds checks, behavior is undefined if index is out of bounds.
:- func unsafe_lookup(mh_ordered_proposition_set, int) = mh_proposition
	is det.
:- pred unsafe_lookup(mh_ordered_proposition_set::in, int::in,
	mh_proposition::out) is det.

	% Search for the value and return the lowest index in the ordered array 
:- func search(mh_ordered_proposition_set, mh_proposition) = int is semidet.
:- pred search(mh_ordered_proposition_set::in, mh_proposition::in, int::out)
	is semidet.

	% Search for the value and return a list of indexes in the ordered array 
	% The 
:- func search_all(mh_ordered_proposition_set::in, mh_proposition::in)
	= (list(int)::out(non_empty_list)) is semidet.
:- pred search_all(mh_ordered_proposition_set::in, mh_proposition::in, 
	list(int)::out(non_empty_list))	is semidet.
...
:- pred first(mh_ordered_proposition_set::in, mh_proposition::out)
	is semidet.
...
:- pred order_by(comparison_func(mh_proposition)::in(comparison_func), 
	mh_ordered_proposition_set::in, mh_ordered_proposition_set::out) is det.
	
:- func order_by(comparison_func(mh_proposition)::in(comparison_func), 
	mh_ordered_proposition_set::in) = (mh_ordered_proposition_set::out)
	is det.
...
:- pred reorder(comparison_func(mh_proposition)::in(comparison_func), 
	mh_ordered_proposition_set::in, mh_ordered_proposition_set::out) is det.
	
:- func reorder(comparison_func(mh_proposition)::in(comparison_func), 
	mh_ordered_proposition_set::in) = (mh_ordered_proposition_set::out)
		is det.
...
% mh_proposition_set before performing them, however, they should preserve the
% leftmost unique ordering of the propositions in the sets, both in the case 
% of union the ordering of the left operand is preserved in the case of 
% intersection and difference

	% The union of two sets
	% appends propositions in the second set not present in the first set to
	% the end of the order in in the first set, does *not* remove duplicates
	% already present in the first set's order, but *does* remove duplicates
	% from the second
:- pred union(mh_ordered_proposition_set::in, mh_ordered_proposition_set::in,
	mh_ordered_proposition_set::out) is det.
:- func union(mh_ordered_proposition_set, mh_ordered_proposition_set)
	= mh_ordered_proposition_set.

	% As above, but removes duplicates from both operands while preserving
	% leftmost ordering of the unique elements
:- pred remove_dups_union(mh_ordered_proposition_set::in, mh_ordered_proposition_set::in,
	mh_ordered_proposition_set::out) is det.
:- func remove_dups_union(mh_ordered_proposition_set,
	mh_ordered_proposition_set) = mh_ordered_proposition_set.


	% The intersection of two sets, preserves the leftmost order of the first
	% set, removes duplicates
:- pred intersect(mh_ordered_proposition_set::in, 
	mh_ordered_proposition_set::in, mh_ordered_proposition_set::out) is det.
:- func intersect(mh_ordered_proposition_set, mh_ordered_proposition_set)
	= mh_ordered_proposition_set.

	% The difference of two sets, preserves the leftmost order of the first
	% set, removes duplicates
:- pred difference(mh_ordered_proposition_set::in, 
	mh_ordered_proposition_set::in, mh_ordered_proposition_set::out) is det.
:- func difference(mh_ordered_proposition_set, mh_ordered_proposition_set)
	= mh_ordered_proposition_set.
...
:- type proposition_array == array(mh_proposition).

:- type mh_ordered_proposition_set 
	--->	ordered_proposition_set(
				order::proposition_array, 
				set::mh_proposition_set
			)
		where comparison is compare_ordered_proposition_sets.

% Deterministic constructor
:- func os(array(mh_proposition), mh_proposition_set) = mh_ordered_proposition_set.
:- mode os(in, in) = out is det.
:- mode os(out, out) = in is det.

:- pragma promise_equivalent_clauses(os/2).

os(O::in, S::in) = (ordered_proposition_set(O, S)::out).

% Deconstructions on types with user defined equality are cc_multi
os(O::out, S::out) = (OS::in) :- 
	promise_equivalent_solutions [O, S] ordered_proposition_set(O, S) = OS. 
...
from_array(A) = os(A, new_proposition_set(A)).
...
:- func unit_map(mh_proposition, _) = unit.
unit_map(_, _) = unit.
...
to_ordered_proposition_set(CMP, Map) = 
...
:- pred intersect_loop(int::in, int::in, proposition_array::in,
	mh_proposition_set::in, mh_proposition_set::in, mh_proposition_set::out,
	proposition_array::array_di, proposition_array::array_uo, int::in,
	int::out) is det.
...
:- pred difference_loop(int::in, int::in, proposition_array::in,
	mh_proposition_set::in, mh_proposition_set::in, mh_proposition_set::out,	
	proposition_array::array_di, proposition_array::array_uo, int::in, 
	int::out) is det.
