...
%-----------------------------------------------------------------------------%
% 
% File: mh_lazy_pattern_map.m
% Main author: C4Cypher.
% Stability: low.
%-----------------------------------------------------------------------------%

:- module mh_lazy_pattern_map.
...
:- func init = (lazy_pattern_map(T)::out) is det.
:- pred init(lazy_pattern_map(_)::out) is det.

:- func singleton(mh_tuple, T) = lazy_pattern_map(T).
:- func unsafe_array_singleton(mh_tuple, array(mh_term), T) = 
	lazy_pattern_map(T).

:- pred is_empty(lazy_pattern_map(_)::in) is semidet.
...
:- pred insert(mh_tuple::in, T::in, lazy_pattern_map(T)::in, 
	lazy_pattern_map(T)::out) is det.

:- pred insert_from_corresponding_lists(list(mh_tuple)::in, list(T)::in,
	lazy_pattern_map(T)::in, lazy_pattern_map(T)::out) is det.
	
:- pred insert_from_assoc_list(assoc_list(mh_tuple, T)::in,
	lazy_pattern_map(T)::in, lazy_pattern_map(T)::out) is det.
	
:- pred unsafe_array_insert(mh_tuple::in, array(mh_term)::in, T::in, 
	lazy_pattern_map(T)::in, lazy_pattern_map(T)::out) is semidet.
...
:- pred set(mh_tuple::in, T::in, lazy_pattern_map::in, lazy_pattern_map::out)
	is det.
...
:- func get_pattern_array(lazy_pattern_map(T), int) = pattern_array(T)).

:- pred get_pattern_array(lazy_pattern_map(T)::in, int::in, 
...
pattern_array_insert(TupleArray, Tuple, T, !.Map, Index) = !:Map :-
	...
	then 
		...
		report_lookup_error(
"mh_lazy_pattern_map.unsafe_array_insert: array aleady present in map", 
		Tuple, !.Map)
	).
