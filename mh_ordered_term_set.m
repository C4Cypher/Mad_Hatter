...
:- func size(ordered_term_set) = int.
:- pred size(ordered_term_set::in, int::out) is det.


% Return the number of unique elements in the set
:- func unique_elements(ordered_term_set) = int.
:- pred unique_elements(ordered_term_set::in, int::out) is det.
...
:- pred bounds(ordered_term_set::in, int::out, int::out) is det.
:- pred set_bounds(ordered_term_set::in, int::out, int::out) is det.

% As above, but fail on emtpy set
:- pred semidet_bounds(ordered_term_set::in, int::out, int::out) is semidet.
:- pred semidet_set_bounds(ordered_term_set::in, int::out, int::out) is semidet.

% Succeed if the given index is in bounds for the given ordered_term_set
:- pred in_bounds(ordered_term_set::in, int::in) is semidet.
:- pred in_set_bounds(ordered_term_set::in, int::in) is semidet.

% Return the first index (should always be 1) unless the set is empty then -1
:- func min(ordered_term_set) = int is det.
:- pred min(ordered_term_set::in, int::out) is det.

:- func semidet_min(ordered_term_set) = int is semidet.
:- pred semidet_min(ordered_term_set::in, int::out) is semidet.

:- func set_min(ordered_term_set) = int is det.
:- pred set_min(ordered_term_set::in, int::out) is det.

:- func semidet_set_min(ordered_term_set) = int is semidet.
:- pred semidet_set_min(ordered_term_set::in, int::out) is semidet.
