...
:- func array_singleton(array(mh_term), T) = tuple_pattern_map(T).
...
:- pred array_insert_from_corresponding_lists(list(array(mh_term))::in, 
	list(T)::in, tuple_pattern_map(T)::in, tuple_pattern_map(T)::out) is det.
...
:- pred array_set_from_corresponding_lists(list(array(mh_term))::in, 
	list(T)::in, tuple_pattern_map(T)::in, tuple_pattern_map(T)::out) is det.	
...
:- pred get_element_map(pattern_array(T)::in, int::in, element_map(T)::out) is 
	det.
...
array_singleton(Array, T) = Map :- array_insert( Array, T, init, Map).

is_empty(init).

:- func insert_pattern(array(mh_term), T, tuple_pattern_map(T)) 
	= tuple_pattern_map(T).
insert_pattern(K, V, !.Map) = !:Map :- array_insert(K, V, !Map).

from_exact_map(Exact) = map_util.fold(insert_pattern, Exact, mh_tuple_pattern_map.init).

%-----------------------------------------------------------------------------%
% Insertion
	
insert(Tuple, T, !Map) :- 
	array_insert(to_array(Tuple), T, !Map).
...
:- func pattern_array_map_insert(array(mh_term), T, element_map(T), int) 
	= element_map(T).
...
:- pragma inline(array_insert/4).

	
array_insert_from_corresponding_lists([], [], !Map).
array_insert_from_corresponding_lists([], [_ | _], _, _) :-
	unexpected($pred, "list length mismatch").
array_insert_from_corresponding_lists([_ | _], [], _, _) :-
    unexpected($pred, "list length mismatch").
array_insert_from_corresponding_lists([K | Ks], [V | Vs], !Map) :-
    array_insert(K, V, !Map),
    array_insert_from_corresponding_lists(Ks, Vs, !Map).
	
:- pragma inline(array_insert_from_corresponding_lists/4).
...
array_set(TupleArray, T, !Map) :-
	array.size(TupleArray, Arity),
	get_pattern_array(!.Map, Arity, OldPatternArray),
	pattern_array_map(pattern_array_set(TupleArray, T), OldPatternArray,
		NewPatternArray),
	set_pattern_array(Arity, NewPatternArray, !Map).
...
pattern_array_set(TupleArray, T, !.Map, Index) = !:Map :-
	array.unsafe_lookup(TupleArray, Index, Term),
	get_exact_map(!.Map, Term, OldExactMap),
	map.set(TupleArray, T, OldExactMap, NewExactMap),
	set_exact_map(Term, NewExactMap, !Map).
	
:- pragma inline(array_set/4).

array_set_from_corresponding_lists([], [], !Map).
array_set_from_corresponding_lists([], [_ | _], _, _) :-
	unexpected($pred, "list length mismatch").
array_set_from_corresponding_lists([_ | _], [], _, _) :-
    unexpected($pred, "list length mismatch").
array_set_from_corresponding_lists([K | Ks], [V | Vs], !Map) :-
    array_set(K, V, !Map),
    array_set_from_corresponding_lists(Ks, Vs, !Map).
	
:- pragma inline(array_set_from_corresponding_lists/4).
...
:- func pattern_array_delete(array(mh_term), element_map(T), int) 
	= element_map(T).

pattern_array_delete(TupleArray, !.Map, Index) = !:Map :-
	array.unsafe_lookup(TupleArray, Index, Term),
	get_exact_map(!.Map, Term, OldExactMap),
	map.delete(TupleArray, OldExactMap, NewExactMap),
	(if map.is_empty(NewExactMap)
	then
		mh_term_map.delete(Term, !Map)
	else
		set_exact_map(Term, NewExactMap, !Map)
	).
...
get_element_map(Array, Index, get_element_map(Array, Index)).

:- pragma inline(get_element_map/3).

pattern_array_is_empty(Array) :- array.all_true(mh_term_map.is_empty, Array).
...
:- mode do_pattern_array_fold(in(func(in, in, in) = out is det), in, in, out, 
	in, in) is det.
:- mode do_pattern_array_fold(in(func(in, in, in) = out is semidet), in, in,
	out, in, in) is semidet.
...
:- pragma inline(det_element_map_insert/5).