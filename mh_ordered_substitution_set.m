...
%-----------------------------------------------------------------------------%
% 
% File: mh_ordered_substitution_set.m
% Main author: C4Cypher.
% Stability: low.
%-----------------------------------------------------------------------------%

:- module mh_ordered_substitution_set.
...
:- type mh_ordered_substitution_set. 
% where comparison is compare_ordered_substitution_sets.
...
% Basic operations
	...
	% Array
:- pred is_valid(mh_ordered_substitution_set::in) is semidet.

:- func empty_set = mh_ordered_substitution_set.

:- pred empty_set(mh_ordered_substitution_set::out) is det.

:- pred is_empty(mh_ordered_substitution_set::in) is semidet.

:- func singleton(mh_substitution) = mh_ordered_substitution_set.

:- pred singleton(mh_substitution::in, mh_ordered_substitution_set::out) is det.

:- pred is_singleton(mh_ordered_substitution_set::in) is semidet.

	% True if the order of the elements are sorted, even with duplicates.
:- pred is_sorted(mh_ordered_substitution_set::in) is semidet.

	% Sort a given set using the standard ordering without removing elements.
:- func sort(mh_ordered_substitution_set) = mh_ordered_substitution_set.
:- pred sort(mh_ordered_substitution_set::in, 
	mh_ordered_substitution_set::out) is det.

	% Sort using the standard ordering and remove duplicaates
:- func sort_and_remove_dups(mh_ordered_substitution_set) 
	= mh_ordered_substitution_set.
:- pred sort_and_remove_dups(mh_ordered_substitution_set::in,
	mh_ordered_substitution_set::out) is det.
	
:- func remove_dups(mh_ordered_substitution_set) = mh_ordered_substitution_set.
:- pred remove_dups(mh_ordered_substitution_set::in, 
	mh_ordered_substitution_set::out) is det.

	% Return the number of elements in the ordered portion of the set
:- func length(mh_ordered_substitution_set) = int.
:- pred length(mh_ordered_substitution_set::in, int::out) is det.


	% Return the number of unique elements in the set
:- func size(mh_ordered_substitution_set) = int.
:- pred size(mh_ordered_substitution_set::in, int::out) is det.

	% Compares the ordered values in the set, including duplicates, succeeds
	% if the set contains the same values in the same order. 
:- pred equal(mh_ordered_substitution_set::in, 
	mh_ordered_substitution_set::in) is semidet.

	% Succeeds if the sets have the same values, with duplicates removed
	% Equivalent to compare_ordered_substitution_sets((=)).
:- pred equivalent(mh_ordered_substitution_set::in, 
	mh_ordered_substitution_set::in) is semidet.
...
:- pred compare_ordered_substitution_sets(comparison_result::uo, 
	mh_ordered_substitution_set::in, mh_ordered_substitution_set::in) is det.

%-----------------------------------------------------------------------------%
% Conversion

:- func from_ordered_set(ordered_set(mh_substitution))
	= mh_ordered_substitution_set.
:- func to_ordered_set(mh_ordered_substitution_set)
	= ordered_set(mh_substitution).

:- func from_list(list(mh_substitution)) = mh_ordered_substitution_set.
:- func to_list(mh_ordered_substitution_set) = list(mh_substitution).

:- func from_array(array(mh_substitution)) = mh_ordered_substitution_set.
:- func to_array(mh_ordered_substitution_set) = array(mh_substitution).

	% Sorts and removes duplicates (if any) from the set, the assigned ordering
	% is ignored
:- func to_sorted_list(mh_ordered_substitution_set) = list(mh_substitution).
:- func to_sorted_array(mh_ordered_substitution_set) = array(mh_substitution).

:- func to_substitution_set(mh_ordered_substitution_set) = mh_substitution_set.
...
:- pred bounds(mh_ordered_substitution_set::in, int::out, int::out) is det.

	% As above, but fail on emtpy set
:- pred semidet_bounds(mh_ordered_substitution_set::in, int::out, int::out)
	is semidet.

	% Succeed if the given index is in bounds for the given ordered_substitution_set
:- pred in_bounds(mh_ordered_substitution_set::in, int::in) is semidet.

	% Return the first index (should always be 1) unless the set is empty 
	% If empty return -1
:- func min(mh_ordered_substitution_set) = int is det.
:- pred min(mh_ordered_substitution_set::in, int::out) is det.

	% Fails if empty
:- func semidet_min(mh_ordered_substitution_set) = int is semidet.
:- pred semidet_min(mh_ordered_substitution_set::in, int::out) is semidet.

	% Return the last index, -1 if empty
:- func max(mh_ordered_substitution_set) = int is det.
:- pred max(mh_ordered_substitution_set::in, int::out) is det.

:- func semidet_max(mh_ordered_substitution_set) = int is semidet.
:- pred semidet_max(mh_ordered_substitution_set::in, int::out) is semidet.

:- pred contains(mh_ordered_substitution_set::in, mh_substitution::in)
	is semidet.

	% Lookup the ordered value at the given index (starting at 1), throws an	% exception if index is out of bounds
:- func lookup(mh_ordered_substitution_set, int) = mh_substitution is det.
:- pred lookup(mh_ordered_substitution_set::in, int::in, mh_substitution::out)
	is det.

	% Skips bounds checks, behavior is undefined if index is out of bounds.
:- func unsafe_lookup(mh_ordered_substitution_set, int) = mh_substitution
	is det.
:- pred unsafe_lookup(mh_ordered_substitution_set::in, int::in,
	mh_substitution::out) is det.

	% Search for the value and return the lowest index in the ordered array 
:- func search(mh_ordered_substitution_set, mh_substitution) = int is semidet.
:- pred search(mh_ordered_substitution_set::in, mh_substitution::in, int::out)
	is semidet.

	% Search for the value and return a list of indexes in the ordered array 
	% The 
:- func search_all(mh_ordered_substitution_set::in, mh_substitution::in)
	= (list(int)::out(non_empty_list)) is semidet.
:- pred search_all(mh_ordered_substitution_set::in, mh_substitution::in, 
	list(int)::out(non_empty_list))	is semidet.
	
	% As above, but return an empty list if the value is not present. 
:- func det_search_all(mh_ordered_substitution_set, mh_substitution)
	= list(int) is det.
:- pred det_search_all(mh_ordered_substitution_set::in, mh_substitution::in,
	list(int)::out) is det.
	
	% return the lowest (leftmost) element, fails if the set is empty
:- func first(mh_ordered_substitution_set) = mh_substitution is semidet.
:- pred first(mh_ordered_substitution_set::in, mh_substitution::out)
	is semidet.

	% throw an exception if the set is empty
:- func det_first(mh_ordered_substitution_set) = mh_substitution is det.
:- pred det_first(mh_ordered_substitution_set::in, mh_substitution::out)
	is det.

	% return the highest (rightmost) element, fails if the set is empty
:- func last(mh_ordered_substitution_set) = mh_substitution is semidet.
:- pred last(mh_ordered_substitution_set::in, mh_substitution::out)
	is semidet.
	
:- func det_last(mh_ordered_substitution_set) = mh_substitution is det.
:- pred det_last(mh_ordered_substitution_set::in, mh_substitution::out)
	is det.
...
:- pred order_by(comparison_func(mh_substitution)::in(comparison_func), 
	mh_ordered_substitution_set::in, mh_ordered_substitution_set::out) is det.
	
:- func order_by(comparison_func(mh_substitution)::in(comparison_func), 
	mh_ordered_substitution_set::in) = (mh_ordered_substitution_set::out)
	is det.
...
:- pred reorder(comparison_func(mh_substitution)::in(comparison_func), 
	mh_ordered_substitution_set::in, mh_ordered_substitution_set::out) is det.
	
:- func reorder(comparison_func(mh_substitution)::in(comparison_func), 
	mh_ordered_substitution_set::in) = (mh_ordered_substitution_set::out)
		is det.
	
	% Given any substitution map (including mh_substitution_set),
	% create an ordering using the given comparison function
:- func to_ordered_substitution_set(
	comparison_func(mh_substitution)::in(comparison_func),
	mh_substitution_map(_)::in) = (mh_ordered_substitution_set::out) is det.
...
% intersection and difference
	...
	% second
:- pred union(mh_ordered_substitution_set::in, mh_ordered_substitution_set::in,
	mh_ordered_substitution_set::out) is det.
:- func union(mh_ordered_substitution_set, mh_ordered_substitution_set)
	= mh_ordered_substitution_set.
...
:- pred intersect(mh_ordered_substitution_set::in, 
	mh_ordered_substitution_set::in, mh_ordered_substitution_set::out) is det.
:- func intersect(mh_ordered_substitution_set, mh_ordered_substitution_set)
	= mh_ordered_substitution_set.

	% The difference of two sets, preserves the leftmost order of the first
	% set, removes duplicates
:- pred difference(mh_ordered_substitution_set::in, 
	mh_ordered_substitution_set::in, mh_ordered_substitution_set::out) is det.
:- func difference(mh_ordered_substitution_set, mh_ordered_substitution_set)
	= mh_ordered_substitution_set.
...
:- type substitution_array == array(mh_substitution).

:- type mh_ordered_substitution_set 
	--->	ordered_substitution_set(
				order::substitution_array, 
				set::mh_substitution_set
			)
		where comparison is compare_ordered_substitution_sets.

% Deterministic constructor
:- func os(array(mh_substitution), mh_substitution_set)
	= mh_ordered_substitution_set.
...
os(O::in, S::in) = (ordered_substitution_set(O, S)::out).

% Deconstructions on types with user defined equality are cc_multi
os(O::out, S::out) = (OS::in) :- 
	promise_equivalent_solutions [O, S] ordered_substitution_set(O, S) = OS. 
...
:- func new_substitution_set(substitution_array) = mh_substitution_set.

:- func set_fold(mh_substitution, mh_substitution_set) = mh_substitution_set
	is det.
	
set_fold(E, !.Map) = !:Map :- set(E, !Map).

new_substitution_set(A) = foldl(set_fold, A, init).

:- pred valid_element(mh_substitution_set::in, mh_substitution::in) is semidet.

valid_element(Set, Sub) :- contains(Set, Sub).
...
compare_ordered_substitution_sets(CMP, os(_, M1), os(_, M2)) :- 
	compare(CMP, M1, M2). 
...
from_array(A) = os(A, new_substitution_set(A)).
...
:- func search_fold(mh_substitution, int, mh_substitution, list(int)) 
	= list(int).
...
:- func to_unsorted_array(mh_substitution_map(_)::in) 
	= (substitution_array::array_uo) is det.

:- func unsorted_insert(mh_substitution, T, unsorted_accumulator) = 
	unsorted_accumulator.

unsorted_insert(Sub, _, usacc(Index, !.Array)) = usacc(Index + 1, !:Array) :-
	unsafe_set(Index, Sub, !Array).
	
to_unsorted_array(Set) = Array :- %probably get a mode error,  
	fold(unsorted_insert, Set, usacc(0, init(count(Set), init)))
		usacc(_, Array).
		
:- func to_unit_map(mh_substitution_map(_)) = mh_substitution_set.

:- func unit_map(mh_substitution, _) = unit.
unit_map(_, _) = unit.
...
to_ordered_substitution_set(CMP, Map) = 
...
:- pred union_and_contains(mh_substitution::in, mh_substitution_set::in, mh_substitution_set::out,
	list(mh_substitution)::in, list(mh_substitution)::out) is det.
	
union_and_contains(Sub, !Union, !Rev) :-
	(if insert(Sub, !Union) % fails if already present 
	then 
		!:Rev = [ Sub | !.Rev], 
...
:- pred intersect_loop(int::in, int::in, substitution_array::in,
	mh_substitution_set::in, mh_substitution_set::in, mh_substitution_set::out,
	substitution_array::array_di, substitution_array::array_uo, int::in,
	int::out) is det.
...
:- pred difference_loop(int::in, int::in, substitution_array::in,
	mh_substitution_set::in, mh_substitution_set::in, mh_substitution_set::out,	
	substitution_array::array_di, substitution_array::array_uo, int::in, 
	int::out) is det.
